...
public class Side_XL_Cannons_8D extends Side_XL{

    public Side_XL_Cannons_8D(boolean isWhite, Game g) {
        super(isWhite, g);
        cannons = new ArrayList(2);
        allKindsOfPieces.add(cannons);
        if (g.LayoutType == Game.XL_PLUS_CANNON_8D) setupPieces();
    }
    ...
    public class Cannon_8D extends Cannon
    {

        public Cannon_8D(int rank, int file) {
            super(rank, file);
        }
        ...
        public boolean canReachTo(Square sqTarget)
        {
            ...
            return directlyReachableOnRankOrFileTo(sqTarget, sq0) || directlyReachableDianal(sqTarget, sq0);
        }
